...
Authentication now uses short-lived access tokens plus an HttpOnly refresh-token cookie. `POST /api/auth/login` and `POST /api/auth/register` issue both tokens, `POST /api/auth/refresh` rotates the cookie and returns a new access token, and `POST /api/auth/logout` revokes the refresh token and clears the cookie. The backend also exposes `POST /api/auth/password-reset/request`, `POST /api/auth/password-reset/confirm`, `POST /api/auth/email-verification/request`, and `POST /api/auth/email-verification/confirm`. The frontend sends credentials with API requests so the refresh cookie can restore a session automatically when an access token expires.
